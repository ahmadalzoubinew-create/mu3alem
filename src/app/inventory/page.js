...
export default function Inventory() {
  ...
  async function handleSave(e) {
    ...
    setLoading(true);
    if (editItem) {
      await supabase.from('inventory').update({ name, stock_quantity: quantity, default_unit: unit }).eq('id', editItem.id);
      setMsg('تم التعديل');
      fetchItems();
      setTimeout(() => { setEditItem(null); setMsg(''); }, 1200);
    } else {
      await supabase.from('inventory').insert({ name, stock_quantity: quantity, default_unit: unit });
      setMsg('تمت الاضافة');
    ...
    <div style={{ minHeight: '100vh', background: '#080808', color: 'white', fontFamily: 'system-ui', paddingBottom: '60px' }}>
      ...
      <div style={{ padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #151515' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '11px', color: '#e8971e', letterSpacing: '2px', textTransform: 'uppercase' }}>Inventory</div>
          <div style={{ fontSize: '20px', fontWeight: 900 }}>المخزون 📦</div>
        </div>
      </div>

      <div style={{ padding: '16px 20px 0', display: 'flex', justifyContent: 'flex-end' }}>
        <button onClick={() => { setShowAdd(true); setEditItem(null); setName(''); setQuantity(''); setUnit('pcs'); }}
          style={{ background: 'rgba(232,151,30,0.15)', border: '1.5px solid #e8971e', borderRadius: '14px', padding: '10px 16px', color: '#e8971e', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}>
          + سلعة جديدة
        </button>
      </div>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        {items.map(item => (
          <div key={item.id} style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => openEdit(item)}
                style={{ background: 'rgba(232,151,30,0.1)', border: '1px solid rgba(232,151,30,0.3)', borderRadius: '10px', padding: '6px 12px', color: '#e8971e', fontSize: '12px', cursor: 'pointer' }}>
                تعديل
              </button>
              <button onClick={() => handleDelete(item.id)}
                style={{ background: 'rgba(206,17,38,0.1)', border: '1px solid rgba(206,17,38,0.3)', borderRadius: '10px', padding: '6px 12px', color: '#CE1126', fontSize: '12px', cursor: 'pointer' }}>
                حذف
              </button>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontWeight: 700, fontSize: '15px' }}>{item.name}</div>
              <div style={{ color: '#444', fontSize: '12px', marginTop: '2px' }}>{item.stock_quantity} {unitLabel[item.default_unit]}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {(showAdd || editItem) && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(0,0,0,0.92)' }}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #161616' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '11px', color: '#e8971e', letterSpacing: '2px', textTransform: 'uppercase' }}>{editItem ? 'Edit Item' : 'New Item'}</div>
                <div style={{ fontSize: '18px', fontWeight: 900, color: 'white' }}>{editItem ? 'تعديل السلعة' : 'سلعة جديدة'}</div>
              </div>
            </div>
              ...
              <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <input type="text" inputMode="decimal" placeholder="الكمية" value={quantity}
                    ...
                    style={{ background: '#161616', border: '1.5px solid #222', borderRadius: '16px', padding: '14px 16px', color: 'white', fontSize: '14px', textAlign: 'right', outline: 'none', width: '100%', boxSizing: 'border-box' }}
                    onFocus={e => e.target.style.borderColor = '#e8971e'}
                    onBlur={e => e.target.style.borderColor = '#222'}
                  />
                  <select value={unit} onChange={e => setUnit(e.target.value)}
                    style={{ background: '#161616', border: '1.5px solid #222', borderRadius: '16px', padding: '14px 16px', color: 'white', fontSize: '14px', outline: 'none', width: '100%', boxSizing: 'border-box' }}>
                ...
                {msg && (
                  <div style={{ textAlign: 'center', fontSize: '13px', fontWeight: 700, color: '#007A3D', padding: '8px', borderRadius: '12px', background: 'rgba(0,122,61,0.1)' }}>
                    {msg}
                  </div>
                )}

                <button type="submit" disabled={loading}
                  style={{ width: '100%', padding: '16px', borderRadius: '16px', border: 'none', background: loading ? '#1a1a1a' : 'linear-gradient(135deg, #e8971e, #c97d10)', color: loading ? '#333' : 'black', fontWeight: 900, fontSize: '14px', cursor: loading ? 'not-allowed' : 'pointer' }}>
