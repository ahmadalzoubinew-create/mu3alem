...
localStorage.setItem(`role_${session.user.id}`, userData?.role || 'salesman');
  ...
  async function handleSave(e) {
    ...
    // upsert — يضيف المندوب إذا ما موجود، أو يحدّث اسمه إذا موجود
    // onConflict: 'id' يضمن ما يتكرر السجل
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', color: 'white',
      ...
      <div style={{ width: '100%', maxWidth: '360px', textAlign: 'center' }}>
        <div style={{ fontFamily: 'Georgia, serif', fontSize: '48px',
          color: '#CE1126', fontWeight: 900, marginBottom: '8px' }}>مُعلم</div>
        <p style={{ color: '#444', fontSize: '12px', marginBottom: '40px',
          letterSpacing: '1px' }}>أول مرة؟ أهلاً 👋</p>

        <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a',
          borderRadius: '24px', padding: '28px 24px' }}>
          <p style={{ color: '#555', fontSize: '14px', textAlign: 'right',
            marginBottom: '20px' }}>
          ...
          <form onSubmit={handleSave}
            ...
            <input type="text" placeholder="مثال: أبو أحمد"
            ...
            <button type="submit" disabled={loading || !displayName.trim()}
              style={{ width: '100%', padding: '16px', borderRadius: '16px',
                border: 'none',
                background: loading || !displayName.trim() ? '#1a1a1a'
                  : 'linear-gradient(135deg, #CE1126, #a00d1e)',
