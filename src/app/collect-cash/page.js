...
export default function CollectCash() {
  ...
  async function handleCollect(e) {
    ...
    const maxDebt = parseFloat(selectedCustomer.total_debt);

    // ── Backend-style guard ──────────────────────────
    if (paid > maxDebt) {
      setAmountError('Fehler: Der Betrag übersteigt die offenen Schulden des Kunden!');
      return;
    }
    setAmountError('');
    setLoading(true);
          ...
          <div>
            ...
            <form onSubmit={handleCollect} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <input
                ...
                required
                style={{ width: '100%', background: '#161616', border: `1.5px solid ${amountError ? '#CE1126' : '#222'}`, borderRadius: '16px', padding: '18px 16px', color: 'white', fontSize: '22px', textAlign: 'right', outline: 'none', boxSizing: 'border-box', fontWeight: 700 }}
              ...
              {/* رسالة الخطأ */}
              {amountError && (
                <div style={{ background: 'rgba(206,17,38,0.08)', border: '1px solid rgba(206,17,38,0.3)', borderRadius: '12px', padding: '10px 14px', textAlign: 'right' }}>
                  <p style={{ color: '#CE1126', fontWeight: 700, fontSize: '12px', margin: 0 }}>⛔ {amountError}</p>
                </div>
              )}
              ...
              <button type="submit" disabled={loading || !!amountError}
                style={{
                  ...
                  background: loading || amountError ? '#1a1a1a' : 'linear-gradient(135deg, #007A3D, #005f2f)',
                  color: loading || amountError ? '#333' : 'white', fontWeight: 900, fontSize: '16px',
                  cursor: loading || amountError ? 'not-allowed' : 'pointer'
                }}>
